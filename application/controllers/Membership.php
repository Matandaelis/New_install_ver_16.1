...
class Membership extends MY_Controller {
	...
	public function plan_create($plan_id = 0){
		...
		$data['levels'] = $this->Product_model->getAll('award_level',false,0,'id desc');
		$data['CurrencySymbol'] = $this->currency->getSymbol();
		$data['plan'] = new \stdClass(); // Initialize empty plan object for new plan creation
	...
	public function plans($page=1){
		...
		$settings = get_pagination_settings(['per_page' => 10, 'preserve_query' => true]);
		$limit = $settings['per_page'];
		$offset = ($page - 1) * $limit;
		
		$query = MembershipPlan::select('membership_plans.*','award_level.level_number as plan_level')->join('award_level','award_level.id','=','membership_plans.level_id','left');
		$total_rows = $query->count();
		$plans = $query->offset($offset)->limit($limit)->get();
		
		$pagination_data = easy_pagination(base_url('membership/plans'), $total_rows, $offset, $settings);
		$data['pagination'] = $pagination_data['html'];
		$data['pagination_summary'] = pagination_summary_html($page, $limit, $total_rows);
	...
	public function buy_membership($plan_id, $paymentGateway = null){
		if($this->Product_model->isMembershipAccess()){
			if($paymentGateway == 'paystack' && !in_array($this->session->userdata('userCurrency'), ['GHS' , 'NGN', 'USD', 'ZAR','KES']))
				show_404();
	...
	public function membership_orders($page = 1){
		...
		$pagination_data = easy_pagination(base_url('membership/membership_orders'), $total_rows, $offset, $settings);
		$data['pagination'] = $pagination_data['html'];
		$data['pagination_summary'] = pagination_summary_html($page, $limit, $total_rows);
		$data['plans'] = $plans;
    ...
    public function getRegularLicenseAlert(){
        ...
        if($details && isset($details['license_type']) && stripos($details['license_type'], 'regular') !== false) {
    ...
    /**
     * Persist the timestamp at which the admin dismissed the regular-license
     * banner. The banner reappears automatically after 7 days.
     */
    public function dismissLicenseNotice() {
        $this->checkLogin('admin');
        $this->Setting_model->save('license_compliance', array(
            'regular_dismissed_at' => time(),
        ));
        $this->json(array('success' => 1));
    }

    /**
     * Persist the one-time acknowledgement that the admin understands a
     * Regular License is not sufficient for the Membership module.
     */
    public function acknowledgeLicenseNotice() {
        $this->checkLogin('admin');
        $this->Setting_model->save('license_compliance', array(
            'extended_acknowledged_at' => time(),
        ));
        $this->json(array('success' => 1));
    }

    /**
     * One-click disable of the Membership module for users who do not hold an
     * Extended License. Writes only the status key so other membership
     * settings are preserved and the operation is fully reversible from the
     * Membership Settings screen.
     */
    public function disableMembershipModule() {
        $this->checkLogin('admin');
        $this->Setting_model->save('membership', array(
            'status' => 0,
        ));
        $this->Setting_model->save('license_compliance', array(
            'membership_disabled_at' => time(),
        ));
        $this->json(array(
            'success' => 1,
            'message' => __('admin.membership_module_disabled'),
        ));
    }
