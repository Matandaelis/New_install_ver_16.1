<?php
/**
 * Starter 2026 — LMS Course Player (standalone layout)
 *
 * @contract  Store API v1 — page: lms_player (standalone — own <html> document)
 * @auth      required (customer must own the course via a completed order)
 *
 * PAGE VARIABLES
 *   $product          array   The course product row (product_name, product_description, etc.)
 *   $chapters         array   Course chapters with lessons [{chapter_title, lessons:[{title, content, video_url}]}]
 *   $order_id         int     Order ID that grants access to this course
 *   $meta_title       string  <title> / OG title for the player page
 *   $meta_description string  OG description
 *   $meta_image       string  OG image URL
 *   $store_setting    array   Store settings (name, logo, etc.)
 */
?>
<!DOCTYPE html>
<html lang="en">
         ...
         <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarCollapse">
               <div class="right-design">
                  ...
                  <div class="login-tab">
                    <a href="<?= base_url('store/my_courses') ?>" title="<?= __('store.my_courses') ?>" style="color:#60c8ff;margin-right:10px"><i class="fa fa-th-large" aria-hidden="true"></i></a>
      ...
      <div class="col-lg-10 ">
         <div class="best mt-2">
            <div class="rating-star">
               ...
               <div class="your-rating py-2 lms-rating-row lms-rating-interactive" id="lmsLessonRating"
                  ...
                  <i class="fa lms-rating-star <?= $initRating >= 1 ? 'fa-star' : 'fa-star-o' ?>" data-value="1" role="button" tabindex="0"></i>
                  <i class="fa lms-rating-star <?= $initRating >= 2 ? 'fa-star' : 'fa-star-o' ?>" data-value="2" role="button" tabindex="0"></i>
                  <i class="fa lms-rating-star <?= $initRating >= 3 ? 'fa-star' : 'fa-star-o' ?>" data-value="3" role="button" tabindex="0"></i>
                  <i class="fa lms-rating-star <?= $initRating >= 4 ? 'fa-star' : 'fa-star-o' ?>" data-value="4" role="button" tabindex="0"></i>
                  <i class="fa lms-rating-star <?= $initRating >= 5 ? 'fa-star' : 'fa-star-o' ?>" data-value="5" role="button" tabindex="0"></i>
...
<script>
   ...
   $( document ).ready(function() { 
      ...
      $(document).on('click', '.lms-rating-star', function (e) {
         ...
         $.ajax({ url: '<?= base_url('store/product_ratting') ?>', type: 'POST', dataType: 'json', data: { product_id: productId, rate: rate },
            success: function (res) {
               if (res.status === 1) updateLmsRatingStars(res.avg_rating);
      ...
      $(document).on('change', '.videocheck', function(event) {
         ...
         var action = 2;  // 2 = explicitly unchecked
         if($(this).is(':checked')) {
            action = 1;
         }
