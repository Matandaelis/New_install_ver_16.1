<?php foreach($orders as $index => $order){ ?>
	<?php if($order['type'] == 'store'){ ?>
		<tr>
			...
			?>
			<td>
				<div class="badge <?= ($order['status'] == 1) ? 'bg-success text-white' : 'bg-warning text-dark' ?>">
				    <?= $status[$order['status']] ?>
				</div>
			</td>
			...
			<td>
				...
				if($order['wallet_commission_status'] == 0) {
					?>
					<span class="badge <?php if((int)$order['wallet_status'] > 0){ ?>bg-success text-white<?php }else{ ?>bg-warning text-dark<?php } ?>"><?= $wallet_status[(int)$order['wallet_status']] ?></span>
					<?php
			 	} else {
		...
		<?php 
			$jsOrders['ex'][$order['id']] =
			    '<ul>'
			    .'<li><b>'. __('admin.order_id').' :</b> <span>'. orderId($order['order_id']).'</span></li>'
			...
			if (!empty($custom_data)) {
			    // Case 1: flat associative array
			    if (array_keys($custom_data) !== range(0, count($custom_data) - 1)) {
			        foreach ($custom_data as $key => $value) {
			            $jsOrders['ex'][$order['id']] .=  
			            '<li class="">'
			                .'<b>' . htmlspecialchars($key) . ':</b> <span>' . htmlspecialchars($value) . '</span>'
			            .'</li>';
			        }
			    } else {
			        // Case 2 and 3: array of arrays
			        foreach ($custom_data as $item) {
			            if (isset($item['key']) && isset($item['value'])) {
			                // Case 2
			                $jsOrders['ex'][$order['id']] .=  
			                '<li class="">'
			                    .'<b>' . htmlspecialchars($item['key']) . ':</b> <span>' . htmlspecialchars($item['value']) . '</span>'
			                .'</li>';
