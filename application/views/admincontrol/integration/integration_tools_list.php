...
<?php foreach ($tools as $key => $tool) { ?>
	<tr class="align-middle">
		...
		<td class="text-center">
		    <img class="rounded shadow-sm intg-thumb" 
		         src="<?= $tool['featured_image'] ?>" 
		         alt="<?= htmlspecialchars($tool['name']) ?>" 
		         data-bs-toggle="modal" data-bs-target="#campaign-details-<?= $tool['id']; ?>">
		</td>

		<!-- Campaign Name -->
		<td>
		    <div>
		        <div class="fw-semibold text-dark mb-1" title="<?= htmlspecialchars($tool['name']) ?>">
		            <?= ucwords(strtolower($tool['name'])) ?>
		        </div>
		        <div class="d-flex align-items-center gap-1 flex-wrap mb-1">
		            <span class="badge bg-<?= $tool['_tool_type'] == 'program' ? 'info' : ($tool['_tool_type'] == 'action' ? 'success' : 'secondary') ?>">
		                <?= parseIntegrationType($tool['_tool_type']) ?>
		            </span>
		            <?php if($tool['username']): ?>
		                <span class="badge bg-success">
		                    <i class="bi bi-person me-1"></i><?= ucwords($tool['username']) ?>
		                </span>
		            <?php else: ?>
		                <span class="badge bg-primary">
		                    <i class="bi bi-shield-lock me-1"></i><?= __('admin.admin') ?>
		                </span>
		            <?php endif; ?>
		            <?php if($tool['category_name']): ?>
		                <span class="badge bg-light text-dark"><?= $tool['category_name'] ?></span>
		            <?php endif; ?>
		        </div>
		        <small class="text-muted">
		            <i class="bi bi-calendar3 me-1"></i><?= date('M d, Y H:i', strtotime($tool['created_at'])) ?>
		        </small>
		    </div>
		</td>
		...
		<!-- Integration Plugin -->
		<td>
		    <?php if(isset($integration_plugins[$tool['tool_integration_plugin']])): ?>
		        <span class="badge bg-primary">
		            <i class="bi bi-plug me-1"></i><?= $integration_plugins[$tool['tool_integration_plugin']]['name'] ?>
		        </span>
		    <?php else: ?>
		        <span class="badge bg-secondary">
		            <i class="bi bi-gear me-1"></i><?= parseIntegrationType($tool['_tool_type']) ?>
		        </span>
		    <?php endif; ?>
		</td>
		...
		<td class="text-center">
		    <span class="badge bg-info fs-6">
		        <i class="bi bi-eye me-1"></i><?= number_format($tool['total_trigger_count']) ?>
		    </span>
		</td>
		...
		<td>
			<?php 
	        	if($tool['_tool_type'] == 'action' || $tool['_tool_type'] == 'single_action')
	        	{
	        		...
	        		echo '<div class="mb-1">';
	        		echo '<span class="badge bg-info text-dark">'.__('admin.action').'</span> ';
	        		echo '<span class="fw-bold">'. $conversionratio . '%</span>';
	        		echo '</div>';
	        	}
	        	if($tool['_tool_type'] == 'general_click')
	        	{
	        		...
	        		$conversionratio = is_float($conversionratio) ==1 ? number_format((float)$conversionratio, 2, '.', '') : $conversionratio;
	        		echo '<div class="mb-1">';
	        		echo '<span class="badge bg-secondary">'.__('admin.click').'</span> ';
	        		echo '<span class="fw-bold">'. $conversionratio . '%</span>';
	        		echo '</div>';
	        	}
	        	if($tool['_tool_type'] == 'program' && $tool['click_status']) 
	        	{
	        		...
	        		$conversionratio = is_float($conversionratio) ==1 ? number_format((float)$conversionratio, 2, '.', '') : $conversionratio;
	        		echo '<div class="mb-1">';
	        		echo '<span class="badge bg-primary">'.__('admin.product_click').'</span> ';
	        		echo '<span class="fw-bold">'. $conversionratio . '%</span>';
	        		echo '</div>';
	        	}
	        	if($tool['_tool_type'] == 'program' && $tool['sale_status'])  
	        	{
	        		...
	        		$conversionratio = is_float($conversionratio) ==1 ? number_format((float)$conversionratio, 2, '.', '') : $conversionratio;
	        		echo '<div>';
	        		echo '<span class="badge bg-success">'.__('admin.sale').'</span> ';
	        		echo '<span class="fw-bold">'. $conversionratio . '%</span> ';
	        		echo '<span class="text-muted">(' . number_format($totalratiocount) . ')</span>';
	        		echo '</div>';
	        	}
	        	 ?>
		</td>
		...
		<td class="text-center security-status">
			<div class="d-flex flex-column align-items-center gap-1">
				...
				<?php if ($im === 's2s_direct'): ?>
				    <span class="badge bg-info text-dark"><i class="bi bi-phone me-1"></i><?= __('admin.integration_type_s2s_mobile') ?></span>
				    <button class="btn btn-sm btn-outline-info btn-show-setup" data-id="<?= $tool['id'] ?>"
				       data-bs-toggle="tooltip" title="<?= __('admin.s2s_mobile_view_setup') ?>">
				        <i class="bi bi-phone"></i>
				    </button>
				<?php elseif ($im === 's2s'): ?>
				    <span class="badge bg-warning text-dark"><i class="bi bi-hdd-rack me-1"></i><?= __('admin.integration_type_s2s') ?></span>
				    <button class="btn btn-sm btn-outline-warning btn-show-setup" data-id="<?= $tool['id'] ?>"
				       data-bs-toggle="tooltip" title="<?= __('admin.s2s_view_setup') ?>">
				        <i class="bi bi-hdd-rack"></i>
				    </button>
				<?php elseif ($im === 'postback'): ?>
				    <span class="badge bg-secondary"><i class="bi bi-arrow-left-right me-1"></i><?= __('admin.integration_type_postback') ?></span>
				    <button class="btn btn-sm btn-outline-secondary btn-show-setup" data-id="<?= $tool['id'] ?>"
				       data-bs-toggle="tooltip" title="<?= __('admin.integration_type_postback') ?>">
				        <i class="bi bi-arrow-left-right"></i>
				    </button>
				<?php elseif ($im === 'conversion_api'): ?>
				    <span class="badge bg-dark"><i class="bi bi-braces me-1"></i><?= __('admin.integration_method_conv_api') ?></span>
				    <button class="btn btn-sm btn-outline-dark btn-show-setup" data-id="<?= $tool['id'] ?>"
				       data-bs-toggle="tooltip" title="<?= __('admin.integration_method_conv_api') ?>">
				        <i class="bi bi-braces"></i>
				    </button>
				<?php else: ?>
				    <span class="badge bg-primary"><i class="bi bi-code-slash me-1"></i><?= __('admin.integration_type_js_pixel') ?></span>
				    <button data-bs-toggle="tooltip" title="<?= __('admin.integration_code') ?>" 
		...
		<td class="text-center">
		    <div class="d-flex align-items-center justify-content-center gap-1">
		        ...
		        <a data-bs-toggle="tooltip" title="<?= __('admin.edit') ?>" 
		           ...
		           onclick="return confirm('<?= __('admin.are_you_sure_to_edit') ?>');" 
		           href="<?= base_url('integration/integration_tools_form/'. $tool['_type'] .'/' . $tool['id']) ?>">
		            <i class="bi bi-pencil-square"></i>
		        </a>

		        <!-- More actions dropdown -->
		        <!-- Popper strategy fixed: avoids menu being clipped by .table-responsive overflow -->
		        <div class="dropdown intg-action-dropdown">
		            <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" data-bs-auto-close="true" data-bs-popper-config='{"strategy":"fixed"}' aria-expanded="false">
		                <i class="bi bi-three-dots-vertical"></i>
		            </button>
		            <ul class="dropdown-menu dropdown-menu-end intg-table-actions-menu">
		                ...
		                <li>
		                    <a class="dropdown-item btn-show-integration-mlm-info" href="javascript:void(0)" data-id="<?= $tool['id'] ?>">
		                        <i class="bi bi-diagram-3 me-2 text-info"></i><?= __('admin.integration_mlm_info') ?>
		                    </a>
		                </li>
		                <?php endif ?>
		                <li>
		                    <a class="dropdown-item btn-campaign-detail" href="javascript:void(0)" 
		                       data-bs-toggle="modal" data-bs-target="#campaign-details-<?= $tool['id']; ?>" data-id="<?= $tool['id']; ?>">
		                ...
		                <li>
		                    <a class="dropdown-item" href="<?= base_url('integration/integration_tools_duplicate/'. $tool['id']) ?>">
		                        <i class="bi bi-copy me-2 text-info"></i><?= __('admin.duplicate') ?>
		                    </a>
		                </li>
		                <li>
		                    <a class="dropdown-item text-danger tool-remove-link" href="<?= base_url('integration/integration_tools_delete/'. $tool['id']) ?>">
	...
	ob_start();
	?>
	<div class="modal fade" id="campaign-details-<?= $tool['id']; ?>" tabindex="-1" aria-labelledby="campaignModal<?= $tool['id'] ?>" aria-hidden="true">
	        ...
	        <div class="modal-content">
	            <div class="intg-modal-header">
	                <div class="intg-modal-header-left">
	                    ...
	                    <div>
	                        <h5 class="intg-modal-title" id="campaignModal<?= $tool['id'] ?>"><?= ucwords(strtolower($tool['name'])) ?></h5>
	                            ...
	                            <div class="row g-2">
	                                <div class="col-md-6">
	                                    <div class="intg-modal-stat">
	                                        ...
	                                        <div>
	                                            <div class="intg-modal-stat-label"><?= __('admin.affiliate_will_get') ?></div>
	                                    ...
	                                    <div class="intg-modal-stat">
	                                        ...
	                                        <div>
	                                            ...
	                                            <div class="intg-modal-stat-value">
	                                                ...
	                                                } else if($tool['commission_type'] == 'fixed'){ 
	                                ...
	                                <div class="col-md-6">
	                                    <div class="intg-modal-stat">
	                                        ...
	                                        <div>
	                                            ...
	                                            <div class="intg-modal-stat-value">
	                                                <?php 
	                                                if($tool['admin_commission_type'] == 'percentage'){ 
	                                                    echo $tool['admin_commission_sale'].'%'; 
	                                                } else if($tool['admin_commission_type'] == 'fixed'){ 
	                                                    echo c_format($tool['admin_commission_sale']); 
	                                                }
	                                                ?>
	                                            </div>
	                                        </div>
...
<script>
$('.postback-info-icon').click(() => Swal.fire({
   title: '<?= __('admin.postback_campaign') ?>',
   text: '<?= __('admin.postback_campaign_no_integration_needed') ?>',
   icon: 'info',
...
$(document).ready(function() {
    ...
    $('#campaign-modals-container').append(modalsHtml);
});
</script>
