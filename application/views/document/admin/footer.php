    ...
    <div class="container-fluid">
        <div class="row align-items-center">
            ...
            <div class="col-md-4 text-center">
                ...
                <a href="<?=base_url();?>api-document" class="btn btn-outline-primary btn-sm">
...
$(document).ready(function() {
    ...
    $('#apiNavList').on('click', '[data-bs-toggle="collapse"]', function(e) {
        ...
        if ($tgt.hasClass('show')) {
            ...
            $t.removeClass('bg-danger text-white').addClass('text-dark');
            $chev.removeClass('bi-chevron-up').addClass('bi-chevron-down');
        } else {
            $tgt.addClass('show').slideDown(200);
            $t.addClass('bg-danger text-white').removeClass('text-dark bg-light');
    ...
    $('#apiNavList').on('click', '.nav-link:not([data-bs-toggle])', function() {
        var $t = $(this), sub = $t.closest('ul').hasClass('ps-3');
        $('#apiNavList .nav-link:not([data-bs-toggle])').removeClass('active bg-danger text-white bg-light').addClass('text-muted');
        $t.addClass('active ' + (sub ? 'bg-light text-dark' : 'bg-danger text-white')).removeClass('text-dark text-muted');
    });
    ...
    // --- Back to Top ---
    var $btt = $('<button class="btn btn-danger rounded-circle shadow d-flex align-items-center justify-content-center position-fixed" id="backToTop" style="width:46px;height:46px;bottom:30px;right:30px;z-index:1050;display:none;"><i class="bi bi-arrow-up fs-5"></i></button>');
    $('body').append($btt);
