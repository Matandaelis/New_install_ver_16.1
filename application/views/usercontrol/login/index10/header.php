...
?>
    ...
    <link href="<?= base_url('assets/login/index10/css/style.css') ?>?v=<?= av() ?>" rel="stylesheet">
    ...
    <?php if (is_rtl()) { ?>
    <link href="<?= base_url('assets/login/index10/css/rtl.css') ?>?v=<?= av() ?>" rel="stylesheet">
    <?php } ?>
    ...
    <style>
        ...
        body, .idx10-title { font-family: <?= $front_side_font_value ?> !important; }
        <?php endif ?>
    </style>
...
<body>
<div class="idx10-page">
    <!-- Left Panel — Description -->
    <div class="idx10-left">
        <div class="idx10-deco-circle idx10-deco-circle-1"></div>
        <div class="idx10-deco-circle idx10-deco-circle-2"></div>
        <div class="idx10-left-inner">
            <div class="idx10-left-pitch">
                <div class="idx10-left-logo">
                    <a href="<?= base_url() ?>">
                        <img src="<?= base_url($logo) ?>" <?= ($SiteSetting['front_custom_logo_size']) ? 'class="customLogoClass"' : '' ?> alt="<?= __('front.logo') ?>">
                    </a>
                </div>
                <div class="idx10-left-content">
                    <h3><?= $setting['heading'] ?></h3>
                    <div class="idx10-left-body"><?= $setting['content'] ?></div>
                </div>
            </div>
            <?php
            $_idx10_aux = trim((string) ($hook_brand_stats ?? '') . (string) ($hook_brand_earners ?? ''));
            if ($_idx10_aux !== '') :
            ?>
            <div class="idx10-brand-blocks">
                <?= $hook_brand_stats ?? '' ?>
                <?= $hook_brand_earners ?? '' ?>
            </div>
            <?php endif; ?>
    ...
    <!-- Right Panel — Form -->
    <div class="idx10-right">
        <div class="idx10-right-topbar">
