...
class Cart extends CI_Model {
	...
	public function getcookieAffiliate(){
		$id = isset($_COOKIE["affiliate_id"]) ? $_COOKIE["affiliate_id"] : null;
	...
	public function setcookieAffiliate($affiliate_id){
		...
		// Validate input - prevent corruption
		if (!is_numeric($affiliate_id) || (int)$affiliate_id < 0) {
			return; // Don't set corrupted cookie
		}
		
		// Improved cookie setting with better reliability for localhost
		$domain = ($_SERVER['HTTP_HOST'] == 'localhost') ? null : $_SERVER['HTTP_HOST'];
		$secure = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on';
		
		setcookie("affiliate_id", $affiliate_id, time()+$time, "/", $domain, $secure, false);
	...
	public function clearCart(){
		...
		$this->db->query("DELETE FROM product_meta WHERE meta_key LIKE 'funnel_cart_" . $this->db->escape_like_str($session_id) . "_%'");
	}
	public function setReferId($refer_id){
		$this->session->set_userdata('refer_id', $refer_id);	
	}
	public function getReferId(){
		$refer_id = (int)$this->session->userdata('refer_id');
		
		// If session is empty, try to restore from persistent storage
		if($refer_id == 0) {
			// Try to get from affiliate_id cookie first
			if(isset($_COOKIE['affiliate_id']) && !empty($_COOKIE['affiliate_id'])) {
				$refer_id = (int)$_COOKIE['affiliate_id'];
				// Restore to session for subsequent requests
				$this->setReferId($refer_id);
			}
			// Fallback to localStorage affiliate (if set by JS)
			else if($this->session->userdata('localStorageAffiliate')) {
				$refer_id = (int)$this->session->userdata('localStorageAffiliate');
				// Restore to session for subsequent requests
				$this->setReferId($refer_id);
			}
		}

		// Lifetime Commission: if still no refer_id, check the customer's stored refid from registration
		if($refer_id == 0) {
			$store_setting = $this->Product_model->getSettings('store');
			if(isset($store_setting['lifetime_commission']) && $store_setting['lifetime_commission'] == 1) {
				$logged_user = $this->is_logged();
				if($logged_user && isset($logged_user['id'])) {
					$customer = $this->db->query("SELECT refid FROM users WHERE id = " . (int)$logged_user['id'])->row();
					if($customer && (int)$customer->refid > 0) {
						$refer_id = (int)$customer->refid;
						$this->setReferId($refer_id);
					}
				}
			}
	...
	public function getProducts($previewId=null){
		...
		$form_id = $this->session->userdata('form_id');
		$isClassified = " product.product_status = 1 AND product.on_store = 1 AND ";
		if (!$form_id) {
			$isClassified .= $store_setting['store_mode'] == 'sales' ? " product.is_campaign_product = 1 AND " : " product.is_campaign_product = 0 AND ";
		}
		...
		$cart_products = $this->db->query("
			...
			WHERE  
			$isClassified session_id = ". $this->db->escape($session_id) ."  $lmsQuery ")->result_array();
		$products  = array();
		...
		foreach ($cart_products as $key => $product) {
			// Check if the image URL is external or from S3.
			if (strpos($product['product_featured_image'], 'http://') === 0 || strpos($product['product_featured_image'], 'https://') === 0 || strpos($product['product_featured_image'], 's3://') === 0) {
			    $product_featured_image = $product['product_featured_image'];
			} else {
			    // If not, it's a local image. Use the base URL to create the full path.
			    $product_featured_image = !empty($product['product_featured_image']) 
			        ...
			        : base_url('assets/images/no_product_image.png');
			}
		...
		$cart_key = 'funnel_cart_' . $session_id . '_' . $product['product_id'];
		$funnel_price_meta = $this->db->select('meta_value')
			->from('product_meta')
			->where('meta_key', $cart_key)
			->where('related_product_id', $product['product_id'])
			->get()
			->row();
		
		if ($funnel_price_meta && !empty($funnel_price_meta->meta_value)) {
			$product['product_price'] = $funnel_price_meta->meta_value;
		}
...
public function getSelectedVariationPrice($product){
	...
	public function getTotals($countryId = null){
		...
		if(!empty($this->allow_shipping)){
			// Check if USPS shipping cost is set in session
			$usps_shipping_cost = $this->session->userdata('shipping_cost');
			
			// Log for debugging
			error_log("Cart getTotals - Session ID: " . $this->session->session_id);
			error_log("Cart getTotals - USPS shipping cost from session: " . var_export($usps_shipping_cost, true));
			error_log("Cart getTotals - allow_shipping: " . var_export($this->allow_shipping, true));
			
			if($usps_shipping_cost !== null && $usps_shipping_cost >= 0){
				// Use USPS shipping cost from session (including 0 for free shipping)
				$_total['shipping_cost'] = array('title' => __('store.shipping_cost'), 'amount' => $usps_shipping_cost);
				error_log("Cart getTotals - Using USPS shipping cost: " . $usps_shipping_cost);
			} else {
				...
				if($rate > 0){
					...
					error_log("Cart getTotals - Using flat rate shipping: " . $rate);
				} else {
					error_log("Cart getTotals - No shipping cost found");
				}
			}
		} else {
			error_log("Cart getTotals - Shipping not allowed");
		}
